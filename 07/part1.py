...
class Hand:
   ...
   def rank_hand(self, cards):
      ...
      if 5 in pile.values(): return 7 # 5 of a kind
      if 4 in pile.values(): return 6 # 4 of a kind
      if 3 in pile.values():
         if 2 in pile.values(): return 5 # Full house
         return 4 # 3 of a kind
      if 2 in pile.values():
         p = [ x for x in pile.values() if x == 2 ]
         if len(p) == 2: return 3 # 2 pair
         return 2 # 1 pair
      return 1 # High card
...
def hand_compare(a: Hand, b: Hand):
   if a == b: return 0
   if a < b: return -1
   if a > b: return 1
