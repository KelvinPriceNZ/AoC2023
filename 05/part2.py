...
while len(seeds) > 0:
   ...
   print(f"New range {start} {seed_range}", file=sys.stderr)
   for seed in range(start, start+seed_range):
      ...
      #print(f"!!! {seed} {link} {goal}", file=sys.stderr)
      if goal < lowest:
         lowest = goal
         print(f"New lowest: {lowest}", file=sys.stderr)
